...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private String myAvatarURL;//自己的头像，为了显示速度，也提成全局变量
	private boolean isGroup;//当前对话界面是不是群聊，为了显示速度，也提成全局变量
	private String hisUserID;//在单聊时候，对方的userid，在群聊时候，群组id
	private String hisRealName;//在单聊时候，对方姓名string
	private AvatarBean hisAvatarBean;//在单聊时候，对方的头像bean，群聊的话该对象无意义
		...
		public void handleMessage(android.os.Message message) {
			...
			case HANDLER_MESSAGE_SELECT_LAST:
                if (messages.length > 0) {
                	//DQ 2016-03-10 原本这里是 messages.length - 1
                    listView.setSelection(messages.length + listView.getFooterViewsCount() - 1);
                }
                break;
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			String gifEmotion = message.getStringAttribute(EaseConstant.MESSAGE_ATTR_EXPRESSION_ID, null);
		    if(gifEmotion!=null){
	...
	public void setChaterInfo(String hisUserID,String hisRealName,AvatarBean hisAvatarBean,String myAvatarURL,boolean isGroup) {
		...
		this.hisAvatarBean = hisAvatarBean;
	...
	public AvatarBean getHisAvatarBean() {
		return hisAvatarBean;
	}
