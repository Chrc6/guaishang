...
public class ListViewEmptyLayout {
	...
	private String mErrorMessage = "数据加载失败，请稍后再试";
	private String mEmptyMessage = "亲，还没有信息哦！";
	...
	private void changeEmptyType() {
		...
		if (!mViewsAdded) {
			...
			if (mEmptyView!=null) rl.addView(mEmptyView,new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
			if (mLoadingView!=null) rl.addView(mLoadingView,new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
			if (mErrorView!=null) rl.addView(mErrorView,new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
			mViewsAdded = true;				
	 ...
	 */
	public ListViewEmptyLayout(Context context, ListView listView) {
		mContext = context;
		mInflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		mListView = listView;
	}
